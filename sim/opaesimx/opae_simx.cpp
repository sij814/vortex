...
#define GEM5_BASE_ADDR 0x20000000
#define PIN_BASE_ADDR 0x10000000
#define PINNED_MEM_SIZE 0xFFFFFF
...
public:
  Impl()
  : ram_(nullptr)
  , pinned_alloc_(0, PINNED_MEM_SIZE, MEM_PAGE_SIZE, CACHE_BLOCK_SIZE)
  , stop_(false)
  , host_buffer_ids_(0)
  {
  ...
  void release_buffer(uint64_t wsid) {
    ...
    if (it != host_buffers_.end()) {
      uint64_t pin_offset = it->second.ioaddr - PIN_BASE_ADDR;
      pinned_alloc_.release(pin_offset);
  ...
  void read_mmio64(uint32_t mmio_num, uint64_t offset, uint64_t *value) {
    ...
    *value = (*(uint64_t*)(GEM5_BASE_ADDR + offset));
  }
  ...
  void write_mmio64(uint32_t mmio_num, uint64_t offset, uint64_t value) {
    ...
    */
    ...
    // simulate mmio request
    (*(uint64_t*)(GEM5_BASE_ADDR + offset)) = value;
  }
...
void opae_simx::write_mmio64(uint32_t mmio_num, uint64_t offset, uint64_t value) {
  impl_->write_mmio64(mmio_num, offset, value);
}

void opae_simx::read_mmio64(uint32_t mmio_num, uint64_t offset, uint64_t *value) {
