...
`endif
    ...
    VX_mem_bus_if.master    mem_bus_if [NUM_BANKS]
);
...
`endif
    ...
    wire [NUM_BANKS-1:0][`CS_MEM_ADDR_WIDTH-1:0]    mem_req_addr_s;
    wire [NUM_BANKS-1:0]                            mem_req_rw_s;
    wire [NUM_BANKS-1:0][LINE_SIZE-1:0]             mem_req_byteen_s;
    wire [NUM_BANKS-1:0][`CS_LINE_WIDTH-1:0]        mem_req_data_s;
    wire [NUM_BANKS-1:0][MEM_TAG_WIDTH-1:0]         mem_req_tag_s;
    ...
    for (genvar i = 0; i < NUM_BANKS; ++i) begin
        VX_elastic_buffer #(
            ...
            .data_out  ({mem_bus_if[i].req_data.rw, mem_bus_if[i].req_data.byteen, mem_bus_if[i].req_data.addr, mem_bus_if[i].req_data.data, mem_bus_if[i].req_data.tag}), 
            .valid_out (mem_bus_if[i].req_valid), 
            .ready_out (mem_bus_if[i].req_ready)
        );
        
        assign mem_bus_if[i].req_data.atype = '0;
    end

    ///////////////////////////////////////////////////////////////////////////

    // Memory response buffering
    wire [NUM_BANKS-1:0]                        mem_rsp_valid_s;
    wire [NUM_BANKS-1:0][`CS_LINE_WIDTH-1:0]    mem_rsp_data_s;
    wire [NUM_BANKS-1:0][MEM_TAG_WIDTH-1:0]     mem_rsp_tag_s;
    wire [NUM_BANKS-1:0]                        mem_rsp_ready_s;
...
`endif
    ...
    for (genvar i = 0; i < NUM_BANKS; ++i) begin
        ...
        `endif
            ...
            .mem_rsp_data       (mem_rsp_data_s[i]),
            .mem_rsp_id         (`CS_MEM_TAG_TO_REQ_ID(mem_rsp_tag_s[i])),
...
`ifdef PERF_ENABLE
    ...
    reg perf_mem_stall_per_cycle_reg;
    for (genvar i = 0; i < NUM_BANKS; ++i) begin
        perf_mem_stall_per_cycle_reg = perf_mem_stall_per_cycle_reg + mem_bus_if[i].req_valid && ~mem_bus_if[i].req_ready;
    end

    wire perf_mem_stall_per_cycle = perf_mem_stall_per_cycle_reg;
