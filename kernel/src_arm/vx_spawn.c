...
#include <vx_spawn_arm.h>
#include <vx_intrinsics_arm.h>
#include <vx_print.h>
...
static void __attribute__ ((noinline)) process_threads() {
  wspawn_threads_args_t* targs = (wspawn_threads_args_t*)cp15_read(VX_CSR_MSCRATCH);
...
static void __attribute__ ((noinline)) process_thread_groups() {
  wspawn_groups_args_t* targs = (wspawn_groups_args_t*)cp15_read(VX_CSR_MSCRATCH);
...
int vx_spawn_threads(uint32_t dimension,
  ...
  if (group_size > 1) {
    ...
    cp15_write(VX_CSR_MSCRATCH, &wspawn_args);
